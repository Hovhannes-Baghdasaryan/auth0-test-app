export const AUTH0_DOMAIN = "react-test-auth0.eu.auth0.com";
export const AUTH0_CLIENT_ID = "wfDJnOehGnLSdQJv9d7syfjZLhaFvP1R";

export const AUTH0_REALM = "Username-Password-Authentication";
export const AUTH0_USER_SCOPE = "openid email profile";

export const AUTH0_LOGIN_RESPONSE_TYPE = "token";
export const AUTH0_LOGOUT_URI = "https://react-auth0-app.herokuapp.com/login";
export const AUTH0_LOGIN_REDIRECT_URI = "https://react-auth0-app.herokuapp.com/authenticate";
